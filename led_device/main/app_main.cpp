...
// Forward declarations of driver APIs
extern esp_err_t app_driver_init(uint16_t light_endpoint_id);
extern esp_err_t app_driver_set_state(bool on);
                                         ...
                                         uint32_t attribute_id, esp_matter_attr_val_t *val, void *priv_data)
{
    if (type == attribute::PRE_UPDATE) {
        if (cluster_id == chip::app::Clusters::OnOff::Id &&
            ...
            ESP_LOGI(TAG, "OnOff attribute change request: %s", val->val.b ? "ON" : "OFF");
...
extern "C" void app_main()
{
    ...
    // Configure and create the On/Off Light endpoint (as a Server)
    ...
    ESP_LOGI(TAG, "On/Off Light endpoint created with ID: %d", endpoint_id);

    // Initialize the WS2812 driver with the endpoint ID
